...
- Use `bun install` to install packages, `bun add` to add new packages, and `bun remove` to remove packages.
...
- When checking out commits ahead of main branch to current branch, use `git log main..HEAD --oneline` to see the commits that will be included in the PR.
- Create PR titles based on the collective changes in the commits ahead of main. Summarize the overall purpose or feature being implemented rather than listing individual commit messages.
- Use descriptive, action-oriented PR titles that explain what the PR accomplishes (e.g., "add user authentication system" rather than "various auth changes").
